        ...
        () => {
          setNameText(
            currentWord.substring(
              0,
              nameText.length + (isNameDeleting ? -1 : 1),
            ),
          );
        },
  ...
  useEffect(() => {
    ...
    if (!isDeleting && text === currentWord) {
      ...
      timeout = setTimeout(
        () => {
          setText(
            currentWord.substring(0, text.length + (isDeleting ? -1 : 1)),
          );
        },
        isDeleting ? 50 : 150,
      );
...
export default function Homepage() {
  return (
    <>
      <div className="bg-background text-foreground/80 font-sans antialiased relative selection:bg-orange-500 selection:text-foreground min-h-screen">
        {/* Decorative Background Blobs for Glassmorphism Effect */}
        <div className="fixed top-[-10%] left-[-10%] w-[40vw] h-[40vw] rounded-full bg-orange-600/20 blur-[120px] pointer-events-none z-0"></div>
        ...
        <div className="relative z-10">
          ...
          <motion.section
            ...
            viewport={{ once: true }}
            className="pt-32 pb-20 container mx-auto px-6 min-h-screen flex flex-col justify-center"
          >
            <div className="flex flex-col lg:flex-row items-center gap-12">
              {/* Left Content */}
              <div className="w-full lg:w-1/2 space-y-6">
                <h3 className="text-lg font-medium text-foreground/60 tracking-wide uppercase min-h-[1.5em]">
                  Hi, I am
                  <span className="text-transparent bg-clip-text bg-linear-to-r from-orange-400 to-orange-600">
                    {" "}
                    <NameTypewriter />
                ...
                <p className="text-foreground/60 leading-relaxed max-w-lg">
                  I build high-performance web applications by blending Figma
                  design and Next.js speed with the robust power of Java Spring
                  Boot and PostgreSQL. I don’t just code; I build seamless
                  digital experiences from the database to the interface. Let’s
                ...
                <div className="flex gap-6 pt-8">
                  <div className="bg-foreground/5 backdrop-blur-md border border-foreground/10 p-4 rounded-2xl text-center flex-1">
                    <h4 className="text-2xl font-bold text-orange-500">0</h4>
                    <p className="text-xs text-foreground/60 uppercase mt-1">
                      Years Exp
                    </p>
                  </div>
                  <div className="bg-foreground/5 backdrop-blur-md border border-foreground/10 p-4 rounded-2xl text-center flex-1">
              ...
              <div className="w-full lg:w-1/2 flex justify-center mt-10 lg:mt-0">
                <div className="relative w-64 h-64 md:w-72 md:h-72 lg:w-96 lg:h-96 mx-auto rounded-full p-2 bg-linear-to-tr from-orange-500/40 to-transparent backdrop-blur-xl border border-foreground/10 shadow-2xl">
